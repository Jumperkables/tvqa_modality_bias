...
__adaptedfrom__ = "Jie Lei"

"""
This is the abstract vs concreete redesign of the TVQA model. The original author of the TVQA pytorch code is Jie Lei. The new adaptions are by Jumperkables. This file isRalso a further adaption from a previous adaption of mine, in the `tvqa_modality_bias` repository
"""
...
import os, sys
hopfield_path = os.path.join(os.path.dirname(__file__), "a-vs-c_models")
sys.path.append(hopfield_path)
import hopfield_layers as hpf
...
class Lxmert_adapt(nn.Module):
    """
    LXMERT ADAPTATION
    """
    def __init__(self, opt):
        super(Lxmert_adapt, self).__init__()
        ...
        # For BERT
        if opt.bert is None:
            raise NotImplementedError("Without BERT not implemented")
        else:
            ...
            elif opt.bert == "lxmertqa":
                raise NotImplementedError("No elegant way to extract outputs")
                lxmert_config = LxmertConfig()
                self.bert = LxmertForQuestionAnswering(lxmert_config)
            elif opt.bert == "lxmert":
                lxmert_config = LxmertConfig()
                self.bert = LxmertModel(lxmert_config)
            #import ipdb; ipdb.set_trace()
            if opt.unfreeze == "all":
                pass
            elif opt.unfreeze == "heads":
                for param in self.bert.base_model.parameters():
                    param.requires_grad = False
            elif opt.unfreeze == "none":
                for param in self.bert.base_model.parameters():
                    param.requires_grad = False
        
        print("Lxmert implementation combines question and answer in the dataloader")
        ...
        if self.vid_flag:
            ...
            raise ValueError("The imagenet, vid, flag must be set as its pivotal to this implementation's design")

        if self.sub_flag:
            raise NotImplementedError("This model isnt considering substitles right now")

        if self.vcpt_flag:
            raise NotImplementedError("This model isnt considering visual concepts right now")           

        if self.reg_flag and not self.regtopk_flag:
            raise NotImplementedError("This model isnt considering regional features right now")

        if self.regtopk_flag:
            raise NotImplementedError("Regional topk features are not considered in this model")
    ...
    def forward(self, q, q_l, a0, a0_l, a1, a1_l, a2, a2_l, a3, a3_l, a4, a4_l,
        ...
        ########
        # Create dummy visual_pos
        vis_pos = torch.tensor([[[0.,0.,479.,639.]]]).repeat(vid.shape[0], vid.shape[1], 1).to(self.opt.device) 
        # visual_cos boxes do not matter according to the huggingface documentation
        #import ipdb; ipdb.set_trace()
        ######## 
        if self.opt.bert is None: # For BERT
            raise NotImplementedError("Not implemented without BERT")
        elif self.opt.bert.startswith("lxmert"):
            qa0 = self.bert(a0, vid, vis_pos)["pooled_output"] # Lxmert implementation combines question and answer in dataloader
            qa1 = self.bert(a1, vid, vis_pos)["pooled_output"]
            qa2 = self.bert(a2, vid, vis_pos)["pooled_output"]
            qa3 = self.bert(a3, vid, vis_pos)["pooled_output"]
            qa4 = self.bert(a4, vid, vis_pos)["pooled_output"]
        catqs = torch.cat([qa0, qa1, qa2, qa3, qa4], dim=1)
        out = self.classifier_fc(catqs)
        if self.opt.lanecheck:
            raise NotImplementedError("Lanechecking isnt relevant")
        else:
            return out
